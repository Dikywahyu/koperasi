$(function () {
    ...
    table.DataTable({
        ...
        buttons: [
            ...
            {
                text: '<i class="ri-add-line ri-16px me-sm-2"></i> <span class="d-none d-sm-inline-block">Tambah Pemasok</span>',
                className:
    ...
    $(document).on("click", ".btn-edit", function () {
        ...
        $.get(`/pemasoks/${id}`, function (data) {
            $("#pemasok-id").val(data.id);
            $("#pemasok-nama").val(data.nama);
            $("#pemasok-kontak").val(data.kontak);
            $("#pemasok-alamat").val(data.alamat);
            $("#pemasok-email").val(data.email);
            offCanvas.show();
        });
    });
    ...
    $("#form-pemasok").on("submit", function (e) {
        e.preventDefault();
        const id = $("#pemasok-id").val();
        const url = id ? `/pemasoks/${id}` : `/pemasoks`;
        ...
        $.ajax({
            ...
            method: method,
            data: {
                nama: $("#pemasok-nama").val(),
                kontak: $("#pemasok-kontak").val(),
                alamat: $("#pemasok-alamat").val(),
                email: $("#pemasok-email").val(),
                _token: $('meta[name="csrf-token"]').attr("content"),
            },
            success: function () {
                table.DataTable().ajax.reload();
